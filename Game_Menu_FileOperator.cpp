	...
	try {
		...
		for (int i = 0, iSize = static_cast<int>(musicDataPathVec.size()); i < iSize; ++i) {
			...
			for (int k = 0; k < difficultySize; ++k) {
				...
				musicData.at(i).push_back(std::make_shared<Game_Menu_MusicData>(Game_Menu_MusicData(musicPath, scorePath,saveDataPath, sjsh.ShiftJISHexToString(std::string(obj.at("name").as_string().c_str())),
					sjsh.ShiftJISHexToString(std::string(obj.at("artist").as_string().c_str())), static_cast<std::uint16_t>(obj.at("level").as_int64()), obj.at("bpm").as_double(),
