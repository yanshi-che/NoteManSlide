...
float Game::Note::Game_Note_LongNoteContainer::noteWidth = 15.0;

Game::Note::Game_Note_LongNoteContainer::Game_Note_LongNoteContainer(std::uint16_t barID,std::uint16_t beatID,const float& y,std::uint8_t amountOfLane,float time) :
...
void Game::Note::Game_Note_LongNoteContainer::setNoteHeight(std::uint8_t laneID,float noteHeight,bool isOne) {
	if (isOne) {
...
const float& Game::Note::Game_Note_LongNoteContainer::getY() {
	return r_y;
}
...
const float& Game::Note::Game_Note_LongNoteContainer::getTime() {
	return time;
}
